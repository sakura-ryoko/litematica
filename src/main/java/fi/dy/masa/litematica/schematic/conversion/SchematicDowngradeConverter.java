...
//    private static final AnsiLogger LOGGER = new AnsiLogger(SchematicDowngradeConverter.class, true, true);
    ...
    public static Optional<NbtCompound> asCompound(NbtElement ele)
    {
        if (ele.getType() == Constants.NBT.TAG_COMPOUND)
        {
            return Optional.of((NbtCompound) ele);
        }

        return Optional.empty();
    }

    public static Optional<NbtList> asNbtList(NbtElement ele)
    {
        if (ele.getType() == Constants.NBT.TAG_LIST)
        {
            return Optional.of((NbtList) ele);
        }
    ...
    {
        NbtCompound oldTags = asCompound(nbtElement).orElse(new NbtCompound());
    ...
    {
        NbtCompound oldTags = asCompound(equipmentEntries).orElse(new NbtCompound());
        NbtCompound newTags = new NbtCompound();
    ...
    {
        NbtCompound oldItem = asCompound(itemEntry).orElse(new NbtCompound());;
    ...
    {
        NbtList oldAttr = asNbtList(attrib).orElse(new NbtList());

        if (oldAttr.isEmpty())
        {
            NbtCompound oldTag = asCompound(attrib).orElse(new NbtCompound());
            ...
            for (String key : oldTag.getKeys())
            {
                ...
                {
                    return processAttributeModifiers(oldTag.getList(key, Constants.NBT.TAG_COMPOUND), minecraftDataVersion, registryManager);
                }
            }
        }
    ...
    private static NbtElement processAttributeBase(NbtList oldAttr, int minecraftDataVersion, DynamicRegistryManager registryManager)
    {
        ...
        for (int i = 0; i < oldAttr.size(); i++)
        {
            ...
            if (attrEntry.contains("type"))
            {
                ...
                newEntry.putDouble("Base", attrEntry.getDouble("amount"));
            }
            else
            {
    ...
    {
        NbtCompound oldNbt = asCompound(nbtIn).orElse(new NbtCompound());
        NbtCompound newNbt = new NbtCompound();

        // todo -- make sure this even needed
//        Codec<Map<RegistryKey<Recipe<?>>, Integer>> CODEC = Codec.unboundedMap(Recipe.CODEC, Codec.INT);
...
//        });
    ...
    {
        NbtCompound origData = asCompound(oldNbt).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldNbt = asCompound(oldEle).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldNbt = asCompound(oldTags).orElse(new NbtCompound());;
...
//        beNbt.copyFrom(newNbt);
    ...
    {
        NbtCompound oldNbt = asCompound(oldPots).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldTag = asCompound(oldDeco).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldTable = asCompound(oldLoot).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldColor = asCompound(oldDye).orElse(new NbtCompound());;
    ...
    {
        NbtList oldNbt = asNbtList(oldProjectiles).orElse(new NbtList());
    ...
    {
        NbtCompound oldUnbr = asCompound(oldNbt).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldNbt = asCompound(oldPot).orElse(new NbtCompound());;
        NbtCompound newNbt = new NbtCompound();
    ...
    {
        NbtCompound oldEnchants = asCompound(oldNbt).orElse(new NbtCompound());;
    ...
    private static String processCustomNameTag(NbtCompound nameTag, String key, @Nonnull DynamicRegistryManager registry)
    {
        ...
        MutableText oldName = BlockEntity.tryParseCustomName(nameTag.getString(key), registry).copy();
    ...
    private static @Nullable MutableText legacyTextSerializer(String json, @Nonnull DynamicRegistryManager registry)
    {
        try
        {
            return TextCodecs.CODEC.parse(registry.getOps(JsonOps.INSTANCE), JsonParser.parseString(json)).getOrThrow().copy();
        }
        catch (Exception err)
        {
    ...
    {
        NbtCompound oldBS = asCompound(bsTag).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldRocket = asCompound(rocket).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldExplosion = asCompound(explosion).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldRecord = asCompound(itemIn).orElse(new NbtCompound());;
    ...
    {
        NbtCompound oldBook = asCompound(bookNbt).orElse(new NbtCompound());;
    ...
    private static NbtCompound processWritableBookContent(NbtCompound bookNbt, int minecraftDataVersion, DynamicRegistryManager registry)
    {
        NbtCompound newBook = new NbtCompound();
...
//                    String newPage = Text.Serialization.toJsonString(oldText, registry);
    ...
    private static NbtCompound processWrittenBookContent(NbtCompound bookNbt, int minecraftDataVersion, DynamicRegistryManager registry)
    {
        NbtCompound newBook = new NbtCompound();
...
//                    String newPage = Text.Serialization.toJsonString(oldText, registry);
    ...
    {
        NbtList oldList = asNbtList(oldPatterns).orElse(new NbtList());
    ...
    {
        NbtCompound profile = asCompound(oldProfile).orElse(new NbtCompound());
...
//        LOGGER.debug("processSkullProfile(): newNBT [{}]", newProfile.toString());
    ...
    {
        NbtList oldBees = asNbtList(beesTag).orElse(new NbtList());
