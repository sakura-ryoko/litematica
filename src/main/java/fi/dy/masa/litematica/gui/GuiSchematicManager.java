	...
	private record ButtonListener(Type type, GuiSchematicManager gui) implements IButtonActionListener
	{
		...
		public void actionPerformedWithButton(ButtonBase button, int mouseButton)
		{
			...
			if (this.type == Type.EDIT_SCHEMATIC)
			{
				if (fileType == FileType.LITEMATICA_SCHEMATIC)
				{
					if (this.gui.editType == EditType.RENAME_SCHEMATIC)
					{
						...
						GuiBase.openGui(new GuiTextInputFeedback(256, "litematica.gui.title.rename_schematic", oldName, this.gui, new SchematicRenamer(entry.getDirectory().toPath(), entry.getName(), this.gui)));
					}
					else if (this.gui.editType == EditType.CHANGE_AUTHOR)
					{
						LitematicaSchematic schematic = LitematicaSchematic.createFromFile(entry.getDirectory(), entry.getName());
						String oldName = schematic != null ? schematic.getMetadata().getAuthor() : "";
						GuiBase.openGui(new GuiTextInputFeedback(128, "litematica.gui.title.schematic_change_author", oldName, this.gui, new SchematicChangeAuthor(entry.getDirectory().toPath(), entry.getName(), this.gui)));
					}
			...
			else if (this.type == Type.FILE_OPS)
			{
				...
				{
					FileDeleter deleter = new FileDeleter(entry.getFullPath().toPath(), this.gui.getListWidget(), Configs.Generic.DISPLAY_FILE_OPS_FEEDBACK.getBooleanValue());
