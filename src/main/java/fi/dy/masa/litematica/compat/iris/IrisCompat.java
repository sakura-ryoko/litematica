...
public class IrisCompat
{
	...
	public static void registerPipelines()
	{
		if (hasIris())
		{
			...
			// TERRAIN_MASA
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.SOLID_TERRAIN_MASA, IrisProgram.TERRAIN_SOLID);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.WIREFRAME_MASA, IrisProgram.LINES);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.CUTOUT_TERRAIN_MASA, IrisProgram.TERRAIN_CUTOUT);

			// TERRAIN_MASA_OFFSET
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.SOLID_TERRAIN_MASA_OFFSET, IrisProgram.TERRAIN_SOLID);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.WIREFRAME_MASA_OFFSET, IrisProgram.LINES);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.CUTOUT_TERRAIN_MASA_OFFSET, IrisProgram.TERRAIN_CUTOUT);
			...
			// BLOCK_MASA
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.SOLID_BLOCK_MASA, IrisProgram.BLOCK);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.CUTOUT_BLOCK_MASA, IrisProgram.TERRAIN_CUTOUT);

			// BLOCK_MASA_OFFSET
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.SOLID_BLOCK_MASA_OFFSET, IrisProgram.BLOCK);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.CUTOUT_BLOCK_MASA_OFFSET, IrisProgram.TERRAIN_CUTOUT);
			...
			// LEGACY_TERRAIN_MASA
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.LEGACY_SOLID_TERRAIN_MASA, IrisProgram.TERRAIN_SOLID);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.LEGACY_WIREFRAME_MASA, IrisProgram.LINES);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.LEGACY_CUTOUT_TERRAIN_MASA, IrisProgram.TERRAIN_CUTOUT);

			// LEGACY_TERRAIN_MASA_OFFSET
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.LEGACY_SOLID_TERRAIN_MASA_OFFSET, IrisProgram.TERRAIN_SOLID);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.LEGACY_WIREFRAME_MASA_OFFSET, IrisProgram.LINES);
			IrisApi.getInstance().assignPipeline(LitematicaPipelines.LEGACY_CUTOUT_TERRAIN_MASA_OFFSET, IrisProgram.TERRAIN_CUTOUT);
