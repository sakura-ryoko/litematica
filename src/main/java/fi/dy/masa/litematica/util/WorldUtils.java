...
public class WorldUtils {
    ...
    public static boolean convertSchematicaSchematicToLitematicaSchematic(
            File inputDir, String inputFileName, File outputDir, String outputFileName, boolean ignoreEntities,
    ...
    public static boolean convertStructureToLitematicaSchematic(File structureDir, String structureFileName,
        ...
        LitematicaSchematic litematicaSchematic = convertStructureToLitematicaSchematic(structureDir,
                structureFileName);
        return litematicaSchematic != null && litematicaSchematic.writeToFile(outputDir, outputFileName, override);
    }
        ...
        try {
            ...
            if (schematic == null) {
                InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,
                        "Failed to read the Sponge schematic from '" + fileName + '"');
            }
    ...
    @Nullable
    public static LitematicaSchematic convertStructureToLitematicaSchematic(File structureDir,
            String structureFileName) {
        try {
            LitematicaSchematic litematicaSchematic = LitematicaSchematic.createFromFile(structureDir,
                    structureFileName, FileType.VANILLA_STRUCTURE);
            ...
            InfoUtils.showGuiOrInGameMessage(MessageType.ERROR,
                    "Exception while trying to load the vanilla structure: " + e.getMessage());
    ...
    public static boolean convertLitematicaSchematicToVanillaStructure(
            File inputDir, String inputFileName, File outputDir, String outputFileName, boolean ignoreEntities,
            boolean override, IStringConsumer feedback) {
        StructureTemplate template = convertLitematicaSchematicToVanillaStructure(inputDir, inputFileName,
                ignoreEntities, feedback);
        return writeVanillaStructureToFile(template, outputDir, outputFileName, override, feedback);
    }

    @Nullable
    public static StructureTemplate convertLitematicaSchematicToVanillaStructure(File inputDir, String inputFileName,
            boolean ignoreEntities, IStringConsumer feedback) {
    ...
    private static boolean writeVanillaStructureToFile(StructureTemplate template, File dir, String fileNameIn,
            boolean override, IStringConsumer feedback) {
    ...
    public static void insertSignTextFromSchematic(SignBlockEntity beClient, String[] screenTextArr, boolean front) {
        ...
        if (worldSchematic != null) {
            ...
            if (beSchem instanceof SignBlockEntity) {
                ...
                SignText textSchematic = front ? beMixinSchem.litematica_getFrontText()
                        : beMixinSchem.litematica_getBackText();
    ...
    public static boolean handleEasyPlace(MinecraftClient mc) {
        if (Configs.Generic.EASY_PLACE_MODE.getBooleanValue() &&
            ...
            if (result == ActionResult.FAIL) {
                MessageOutputType type = (MessageOutputType) Configs.Generic.PLACEMENT_RESTRICTION_WARN
                        .getOptionListValue();
    ...
    private static ActionResult doEasyPlaceAction(MinecraftClient mc) {
        ...
        if (Configs.Generic.EASY_PLACE_FIRST.getBooleanValue()) {
            ...
            traceWrapper = RayTraceUtils.getFurthestSchematicWorldTraceBeforeVanilla(mc.world, mc.player,
                    traceMaxRange);
        ...
        if (traceWrapper.getHitType() == RayTraceWrapper.HitType.SCHEMATIC_BLOCK) {
            ...
            if (stack.isEmpty() == false) {
                ...
                // If there is a block in the world right behind the targeted schematic block,
                // then use
     ...
     * If the targeted position is outside of the current layer range, or should be
     * air
     * in the schematic, or the player is holding the wrong item in hand, then true
     * is returned
        ...
        switch (axis) {
            case Z: {
                ...
                for (int cx = cxMin; cx <= cxMax; ++cx) {
                    ...
                    int yMax = Math.min(y2, yMaxMath + 15);
    ...
    private static boolean easyPlaceIsTooFast() {
        return System.nanoTime() - easyPlaceLastPickBlockTime < 1000000L
                * Configs.Generic.EASY_PLACE_SWAP_INTERVAL.getIntegerValue();
    }
